...
auto Count(std::convertible_to<TLinkAddress> auto... restrictions) const
    -> TLinkAddress
...
auto Exists(TLinkAddress link) const noexcept -> bool {
  ...
  return IsExternalReference(constants, link) ||
         (IsInternalReference(constants, link) &&
          links.Count(LinkAddress(link)) != 0);
}

auto EnsureLinkExists(TLinkAddress link, const std::string &argument = {}) const
    -> void {
  auto &&links = *this;
...
auto Each(auto &&handler,
          std::convertible_to<TLinkAddress> auto... restrictions) const
    -> TLinkAddress
...
auto GetLink(TLinkAddress link) const -> Interfaces::IArray auto{
  ...
  // auto linkPartsSetter = Setter<IList<TLinkAddress>,
  ...
  links.Each(
      [&wrapper, &constants](Interfaces::IArray auto &&link) {
